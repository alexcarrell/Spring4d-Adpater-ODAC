...
uses
  ...
  Spring.Reflection,
  Spring.TestUtils,
  TestDevartODACConnection;

type
  IDevArtOdacExecuteCall = interface
  end;


  TBaseDevArtOdacAdapterTest = class(TTestCase)
  protected
    fMockConnectionObject: Mock<TTestODACConnection>;
    procedure SetUp; override;
    procedure TearDown; override;
    procedure SetupOpen;
    //function SetupExecute(const queries: array of string): IDevArtOdacExecuteCall;
  end;
  (*
...
type
  TDevArtODACExecuteCall = class(TInterfacedObject, IDevArtOdacExecuteCall)
  private
...
constructor TDevArtODACExecuteCall.Create(const testCase: TTestCase;
  const queries: array of string);
begin
  inherited Create;
  fTestCase := testCase;
...
function TDevArtODACExecuteCall.Execute(const callInfo: TCallInfo): TValue;
var
  query: string;
begin
...
{$REGION 'TBaseDevArtOdacAdapterTest'}

procedure TBaseDevArtOdacAdapterTest.SetUp;
begin
  inherited;
  fMockConnectionObject := Mock<TTestODACConnection>.Create(TMockBehavior.Strict);
end;

(*
function TBaseDevArtOdacAdapterTest.SetupExecute(const queries: array of string): IDevArtODACExecuteCall;
var
  lResult: TDevArtODACExecuteCall;
  o: OleVariant;
begin
  lResult := TDevArtODACExecuteCall.Create(Self, queries);
  Result := lResult;
  o := fMockConnectionObject.Setup.Executes(lResult.Execute).When(Args.Any)
    .ExecSQL('');
end;
 *)
procedure TBaseDevArtOdacAdapterTest.SetupOpen;
begin
  with fMockConnectionObject.Setup.Executes do
  begin
    When(Args.Any).ConnectString;
//    When(Args.Any).State;
    When(Args.Any).Open('', '', '', 0);
  end;
end;

procedure TBaseDevArtOdacAdapterTest.TearDown;
begin
  inherited;
